...
#SBATCH --output=logs/train_wan_low_%j.out
#SBATCH --error=logs/train_wan_low_%j.err
...
srun --label --export=ALL --ntasks-per-node=1 --gres=gpu:4 \
    ...
    wpd.sif bash -c "
        ...
        --output_path "./models/train/Wan2.2-I2V-A14B_low_noise_lora_wpd" \
        --lora_base_model "dit" \
