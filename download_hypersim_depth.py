"""
Download Hypersim GT depth maps (depth_meters.hdf5) from the same scene zips
used for RGB download. Streams zips via HTTP range requests and extracts only
the geometry_hdf5 files for frames 0000-0009.

Output flat dir: /mrtstorage/datasets_tmp/hypersim_depth/
  ai_001_001_cam00_frame0000.hdf5
"""
...
OUT_DIR = Path("/mrtstorage/datasets_tmp/hypersim_depth")
OUT_DIR.mkdir(parents=True, exist_ok=True)

FRAMES = {f"frame.{i:04d}.depth_meters.hdf5" for i in range(10)}

# Same URLs as download_hypersim_first_frames.py
URLS = [
...
class WebFile:
    ...
    def read(self, n=-1):
        end = self.size - 1 if n == -1 else min(self.pos + n - 1, self.size - 1)
        if self.pos > end:
            return b""
...
def flat_name(zip_member):
    ...
    # → ai_001_001_cam00_frame0000.hdf5
    m = re.match(
        r"(ai_\d+_\d+)/images/scene_cam_(\d+)_geometry_hdf5/(frame\.\d+)\.depth_meters\.hdf5",
        zip_member
    )
    if not m:
        return None
    ...
    return f"{scene}_cam{cam}_frame{frame_num}.hdf5"


def process_scene(url, session, total, idx):
    scene = url.split("/")[-1].replace(".zip", "")

    existing = list(OUT_DIR.glob(f"{scene}_cam*_frame0000.hdf5"))
    if existing:
    ...
    try:
        ...
        with zipfile.ZipFile(wf) as zf:
            members = [
                ...
                if "geometry_hdf5" in m and any(f in m for f in FRAMES)
            ]
            saved = 0
            for m in members:
                name = flat_name(m)
                ...
                saved += 1
        print(f"  → {saved} depth maps saved", flush=True)
        return saved
...
def main():
    ...
    all_files = list(OUT_DIR.glob("*.hdf5"))
    print(f"\nDone. Total depth maps: {len(all_files)}")
