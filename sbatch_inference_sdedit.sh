#!/bin/bash
#SBATCH --job-name=inference_sdedit
#SBATCH --nodes=1
...
#SBATCH --output=logs/inference_sdedit_%j.out
#SBATCH --error=logs/inference_sdedit_%j.err

# NOTE: You must run 'mkdir -p logs' manually in your terminal before sbatch!

# Check if an argument was provided
if [ -z "$1" ]; then
    echo "Error: No argument provided. Usage: sbatch sbatch_inference_sdedit.sh <timestep>"
    exit 1
fi
...
srun --label --export=ALL \
    ...
    wpd.sif bash -c "
        cd /workspace && \
        PYTHONPATH=. \
        torchrun --nproc_per_node=4 examples/flux/model_inference/FLUX.1-dev_sdedit_synthia.py \
        --synthia_folder /workspace/data/synthia \
        --timestep \"$1\" \
        --output_folder /workspace/data/synthia_sdedit_${1}"