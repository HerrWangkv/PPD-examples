...
#SBATCH --output=logs/inference_ppd_%j.out
#SBATCH --error=logs/inference_ppd_%j.err

# NOTE: You must run 'mkdir -p logs' manually in your terminal before sbatch!

# Check if an argument was provided
if [ -z "$1" ]; then
    echo "Error: No argument provided. Usage: sbatch sbatch_inference_baseline.sh <radius>"
    exit 1
fi
...
srun --label --export=ALL \
    ...
    wpd.sif bash -c "
        ...
        --radius \"$1\" \
        --output_folder /workspace/data/synthia_ppd_${1}"