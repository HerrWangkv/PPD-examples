#!/bin/bash
#SBATCH --job-name=wan_high_openscene
#SBATCH --nodes=1
...
#SBATCH --output=logs/train_wan_high_openscene_%j.out
#SBATCH --error=logs/train_wan_high_openscene_%j.err
...
srun --label --export=ALL --ntasks-per-node=1 --gres=gpu:4 \
    ...
    wpd.sif bash -c "
        cd /workspace && \
        PYTHONPATH=. \
        torchrun --nproc_per_node=4 --master_port=29500 examples/wanvideo/model_training/train_wan_openscene.py \
        --height 704 \
        --width 1280 \
        ...
        --output_path \"./models/train/Wan2.2-I2V-A14B_high_noise_lora_wpd_openscene\" \
        --lora_base_model \"dit\" \
