#!/bin/bash
#SBATCH --job-name=inference_hypersim_ppd
#SBATCH --nodes=1
...
#SBATCH --output=logs/inference_hypersim_ppd_%j.out
#SBATCH --error=logs/inference_hypersim_ppd_%j.err
#SBATCH --signal=B:USR1@300

# Hypersim sim-to-real translation: PPD r=20, 4-GPU torchrun, with resume support.
# Input:  /hkfs/work/workspace/scratch/xw2723-generation/hypersim   (7402 frames)
# Output: /hkfs/work/workspace/scratch/xw2723-generation/hypersim_wpd/ppd_r20
#
# Usage: sbatch sbatch_inference_hypersim_ppd.sh
# For accelerated (48h): sed 's/00:10:00/48:00:00/;s/dev_accelerated/accelerated/' sbatch_inference_hypersim_ppd.sh | sbatch
...
chain_resubmit() {
    ...
    NEXT=$(sbatch --parsable \
           ...
           "${WORKSPACE}/sbatch_inference_hypersim_ppd.sh")
...
srun --label --export=ALL \
    ...
    ${SIF} bash -c "
        ...
        torchrun --nproc_per_node=4 batch_sim2real_image_ppd.py \
            ...
            --output_dir /data/hypersim_wpd/ppd_r20 \
            --flux_lora ${PPD_LORA} \
            --flux_cutoff_radius 20 \
            --prompt '${PROMPT}' --negative_prompt '${NEG}'
        echo 'Hypersim PPD r=20 done.'
    " &
