"""
Stream Hypersim scene zips and extract frames 0000-0009 from final_preview
into a flat directory: hypersim/ai_001_001_cam00_frame0000.jpg
"""
...
OUT_DIR = Path("/mrtstorage/datasets_tmp/hypersim")
OUT_DIR.mkdir(parents=True, exist_ok=True)

FRAMES = {f"frame.{i:04d}.color.jpg" for i in range(10)}
...
class WebFile:
    """HTTP range-request backed file-like object for streaming zip access."""
...
def flat_name(zip_member):
    # ai_001_001/images/scene_cam_00_final_preview/frame.0000.color.jpg
    # → ai_001_001_cam00_frame0000.jpg
    m = re.match(
        r"(ai_\d+_\d+)/images/scene_cam_(\d+)_final_preview/(frame\.\d+)\.color\.jpg",
        zip_member
    )
    if not m:
        return None
    scene, cam, frame = m.group(1), m.group(2), m.group(3)
    frame_num = frame.split(".")[1]  # "0000"
    return f"{scene}_cam{cam}_frame{frame_num}.jpg"


def process_scene(url, session, total, idx):
    scene = url.split("/")[-1].replace(".zip", "")

    # Skip if all 10 frames already downloaded (assuming single cam; check at least frame0000)
    existing = list(OUT_DIR.glob(f"{scene}_cam*_frame0000.jpg"))
    if existing:
        print(f"[{idx}/{total}] SKIP {scene}")
        return 0

    print(f"[{idx}/{total}] {scene}", flush=True)
    try:
        wf = WebFile(url, session)
        with zipfile.ZipFile(wf) as zf:
            members = [
                m for m in zf.namelist()
                if "final_preview" in m and any(f in m for f in FRAMES)
            ]
            saved = 0
            for m in members:
                name = flat_name(m)
...
def main():
    ...
    with requests.Session() as session:
        for idx, url in enumerate(URLS, 1):
            process_scene(url, session, total, idx)

    all_files = list(OUT_DIR.glob("*.jpg"))
