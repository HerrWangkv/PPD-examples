#!/bin/bash
#SBATCH --job-name=inference_controlnet
#SBATCH --nodes=1
...
#SBATCH --output=logs/inference_controlnet_%j.out
#SBATCH --error=logs/inference_controlnet_%j.err

# NOTE: You must run 'mkdir -p logs' manually in your terminal before sbatch!

# Check if an argument was provided
if [ -z "$1" ]; then
    echo "Error: No argument provided. Usage: sbatch sbatch_inference_controlnet.sh <scale>"
    exit 1
fi
...
srun --label --export=ALL \
    ...
    wpd.sif bash -c "
        cd /workspace && \
        PYTHONPATH=. \
        torchrun --nproc_per_node=4 examples/flux/model_inference/FLUX.1-dev-Controlnet-Union-alpha_synthia.py \
        --synthia_folder /workspace/data/synthia \
        --scale \"$1\" \
        --output_folder /workspace/data/synthia_controlnet_${1}"