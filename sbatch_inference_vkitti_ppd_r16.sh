#!/bin/bash
#SBATCH --job-name=vkitti_ppd_r16
#SBATCH --nodes=1
...
#SBATCH --output=logs/vkitti_ppd_r16_%j.out
#SBATCH --error=logs/vkitti_ppd_r16_%j.err
#SBATCH --signal=B:USR1@300

# PPD r=16 on vKITTI clone frames, 4-GPU torchrun, with resume support.
# Input:  /hkfs/work/workspace/scratch/xw2723-generation/vkitti/clone_flat (2126 frames)
# Output: /hkfs/work/workspace/scratch/xw2723-generation/vkitti_wpd/ppd_r16
...
chain_resubmit() {
    ...
    NEXT=$(sbatch --parsable \
           ...
           "${WORKSPACE}/sbatch_inference_vkitti_ppd_r16.sh")
...
srun --label --export=ALL \
    ...
    ${SIF} bash -c "
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29500 \
            ...
            > /workspace/logs/vkitti_ppd_r16_\${SLURM_JOB_ID}.out 2>&1
    " &
SRUN_PID=$!
