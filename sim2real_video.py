...
def load_depth_frames_as_tensor(video_path, height, width, n_frames, device):
    ...
    disparity_tensor = torch.from_numpy(disparity_norm).unsqueeze(1).to(device)
...
def run_wan_stage(args, first_frame_gen, rgb_frames_pil, disparity_video_tensor, device):
    ...
    with torch.no_grad():
        ...
        # Input disparity is (T, 1, H, W). 
        # Permute to (1, 1, T, H, W) for interpolation
        target_disparity = disparity_video_tensor[:args.n_frames].permute(1, 0, 2, 3).unsqueeze(0).float()
        ...
        # Sky Mask Logic (from train_wan_openscene.py)
        ...
        # and the WPD Noise (derived from original video motion) as the starting noise.
        video = pipe(
            prompt=args.prompt,
...
if __name__ == "__main__":
    ...
    disparity_tensor = load_depth_frames_as_tensor(args.depth_video, args.height, args.width, args.n_frames, device)
