#!/bin/bash
#SBATCH --job-name=vkitti_wpd_baseline_r10
#SBATCH --nodes=1
...
#SBATCH --output=logs/vkitti_wpd_baseline_r10_%j.out
#SBATCH --error=logs/vkitti_wpd_baseline_r10_%j.err
#SBATCH --signal=B:USR1@300

# WPD baseline (flux.safetensors, no drop_ll) r=10 on vKITTI clone frames, 4-GPU torchrun.
# Input:  /hkfs/work/workspace/scratch/xw2723-generation/vkitti/clone_flat (2126 frames)
# Output: /hkfs/work/workspace/scratch/xw2723-generation/vkitti_wpd/wpd_baseline_r10
...
chain_resubmit() {
    ...
    NEXT=$(sbatch --parsable \
           ...
           "${WORKSPACE}/sbatch_inference_vkitti_wpd_baseline_r10.sh")
...
srun --label --export=ALL \
    ...
    ${SIF} bash -c "
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29500 \
            ...
            --output_dir /data/vkitti_wpd/baseline_r10 \
            --flux_lora ${BASELINE_LORA} \
            --flux_cutoff_radius 10 \
            --height 384 --width 1280 \
            --prompt '${PROMPT}' --negative_prompt '${NEG}' \
            > /workspace/logs/vkitti_wpd_baseline_r10_\${SLURM_JOB_ID}.out 2>&1
    " &
SRUN_PID=$!
